...
public sealed class YubiWireProtocolTests
{
    ...
    public void PoseFrameIsUtf8JsonWithExactlyOneTrailingNewline()
    {
        ...
        byte[] encoded = YubiWireProtocol.EncodePoseFrame(pose);
    ...
    public void TimeSyncRequestAndResponseMatchLittleEndianLayout()
    {
        ...
        Assert.That(YubiWireProtocol.TryParseTimeSyncRequest(request, out long parsed),
            Is.True);
        Assert.That(parsed, Is.EqualTo(pcTime));

        byte[] response = YubiWireProtocol.EncodeTimeSyncResponse(pcTime, questTime);
    ...
    {
        Assert.That(YubiWireProtocol.TryParseTimeSyncRequest(request, out _), Is.False);
    }

    [Test]
    public void DefaultPortsMatchHandumiLegacyConfiguration()
    {
        Assert.That(YubiWireProtocol.PosePort, Is.EqualTo(65432));
        Assert.That(YubiWireProtocol.TimeSyncPort, Is.EqualTo(42000));
    }

    [Test]
    public void StatusTextOnlyShowsIpForConnectedClients()
    {
        Assert.That(
            QuestStatusDisplay.FormatStatus(false, "192.168.1.20"),
            Is.EqualTo("HandUMI Quest App"));
        Assert.That(
            QuestStatusDisplay.FormatStatus(true, "192.168.1.20"),
            Is.EqualTo("HandUMI Quest App\nConnected • IP: 192.168.1.20"));
    }
    ...
    public void AndroidIdentitySupportsSideBySideInstall()
    {
        ...
        Assert.That(PlayerSettings.productName,
            Is.EqualTo("HandUMIQuestApp-v0.1.0"));
