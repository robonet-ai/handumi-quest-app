# HandUMI Quest App reference APK analysis

## Bottom line

The uploaded file is the reference Quest client. It is a Unity **IL2CPP** build, so the original C# files are not present as readable assemblies. Static analysis nevertheless recovers the exact app-script filenames, custom class and member names, serialized data schema, default ports, major runtime behavior, build/toolchain metadata, and the complete wire protocol.
...
The public bridge uses the same field keys and describes the current transport as legacy TCP/JSON, with a future HandUMI UDP/binary transport planned.
